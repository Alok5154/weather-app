...
function Weather(props: any) {
    ...
    const locationKey = props?.data?.locations ? Object.keys(props.data.locations)[0] : null;
    const location = locationKey ? props.data.locations[locationKey] : null;

    if (!location || !Array.isArray(location.values) || location.values.length === 0) {
        return <div className="text-center text-white py-20">Weather data is unavailable right now.</div>
    }
